function tab_edit(Num) {
    ...
    var SelectBtm = document.querySelectorAll('.but' + Num);
    SelectBtm.forEach(function (button) {
        button.classList.add('selected');
    });

    var par = document.querySelector('.content_me');

    switch (Num) {
        case '1':
            par.innerHTML = `<div class="text1">
                <p> Я Студент группы ПР-21<br>Сивков Владислав Сергеевич<br>Закончил СПО с красным
                    дипломом<br>и продолжаю обучение на высшем образовании</p>
                <p>Владислав родился в 2002 году в городе Пермь. С детства проявлял интерес к математике и
                    естественным наукам. <br> В 2018 году поступил в СПО на техническую специальность.
                    За время обучения проявил себя как ответственный и целеустремленный студент,
                    закончив обучение с
                    красным дипломом. <br> В настоящее время продолжает обучение на высшем образовании.</p></div>`;
            break;
        case '2':
            par.innerHTML = `<div class="text2">
                <img src="src/ME/dip1.jpg" alt="Фото примера сертификата">
                <img src="src/ME/dip2.jpg" alt="Фото примера сертификата">
                <img src="src/ME/dip3.jpg" alt="Фото моего сертификата">
                </div>`;
            break;
        case '3':
            par.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 790 563" fill="none">
            <g id="Image">
                <g id="g14">
                    <g id="g16">
                        <g id="g22">
        ...
        <form>
                    <h1 class="title text-center mb-4">Оставьте отзыв о сайте...</h1>
                    ...
                    <div class="form-group position-relative">
                        ...
                        <input type="text" id="formName" class="form-control form-control-lg thick" placeholder="Имя">
                    </div>
                    ...
                    <div class="form-group message">
                        <textarea id="formMessage" class="form-control form-control-lg" rows="7" placeholder="Сообщение"></textarea>
                    </div>
            
                    <!-- Submit btn -->
                    <div class="text-center">
                        <button type="submit" class="btn btn-primary" tabIndex="-1" OnClick="OK_Sent()">Оставить комментарий</button>
                    </div>
                </form>`;
            break;
        default:
            par.innerHTML = `<div><H1>Ошибка! Как так?</H1></div>`
    }
}

function OK_Sent() {
    alert("Вижу что написано сообщение, но обработка не до настроена))");
    tab_edit('3');
}